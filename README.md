# README for `genpipes_chipseq_report_with_wrapper`
...
4. On line 173 of `generate_report_wrapper.sh`, edit to the path to `generate_report.sh` accordingly.
...
1. Go to `report` directory of GenPipes' output.
2. Run `generate_report_wrapper.sh` from the login node.\
Example:
```bash
...
4. Check inputs and press Enter to confirm submission of `generate_report.sh` to SLURM.
